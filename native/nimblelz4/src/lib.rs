...
// Capacity (in chunks) of the bounded channel feeding compressed data to the
// decoder thread. When the caller feeds data faster than the decoder can
// consume it, `send` blocks instead of letting the queue grow without limit,
// which is what keeps memory usage bounded regardless of the total size of the
// stream.
const DECOMPRESS_INPUT_CAPACITY: usize = 64;
...
//
// The input channel is bounded: `update`'s `send` blocks once the decoder is
// `DECOMPRESS_INPUT_CAPACITY` chunks behind, applying backpressure so the caller
// cannot outrun the decoder and grow the queue with the total size of the
// stream. The output channel is intentionally left unbounded: a single LZ4
// frame block (up to several MB) can be buffered inside the decoder and flushed
// as many output reads without consuming any further input, so a bounded output
// channel could deadlock (decoder blocked emitting output, caller blocked
// feeding input, neither able to free the other). With the input bounded, the
// decoder can only ever run `DECOMPRESS_INPUT_CAPACITY` chunks ahead, so the
// output it buffers is bounded by a constant, not by the length of the stream.
...
fn decompress_stream_new() -> ResourceArc<DecompressStream> {
    let (input_tx, input_rx) = mpsc::sync_channel::<Vec<u8>>(DECOMPRESS_INPUT_CAPACITY);
...
fn decompress_stream_update<'a>(
    ...
    // Grab a handle to the sender without holding the lock across the
    // (potentially blocking) send below.
    let tx = {
    ...
    // Feed the decoder. This blocks if the input channel is full, applying
    // backpressure so the caller cannot outrun the decoder. An error means the
    // decoder thread has gone away, which we surface while draining below.
    let _ = tx.send(data.as_slice().to_vec());

    // Return whatever output is ready, without blocking.
    let mut collected: Vec<u8> = Vec::new();
    match drain_ready_output(&resource.output, &mut collected) {
        Ok(()) => Ok((atom::ok(), binary_from_slice(env, &collected)).encode(env)),
        Err(e) => Ok((atom::error(), e).encode(env)),
    }
}
...
/// Non-blocking drain of the decoder's output channel into `collected`.
/// Returns `Err` with the decoder's message if it reported a decode error.
fn drain_ready_output(
    ...
    loop {
        match output.try_recv() {
            Ok(DecompressMsg::Chunk(chunk)) => collected.extend_from_slice(&chunk),
            Ok(DecompressMsg::Error(e)) => return Err(e),
